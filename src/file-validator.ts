    ...
    switch (mode) {
      case "write": {
        ...
        for (const missingNamespace of missingNamespaces) {
          const filePath = path.join(
            config.localeRoot,
            missingNamespace.locale,
            `${missingNamespace.namespace}.json`,
          );
      ...
      case "print": {
        ...
        missingNamespaces.forEach((missingNamespace) => {
          console.log(
            `  - ${missingNamespace.namespace} in ${missingNamespace.locale}`,
          );
        });
        break;
      }
  ...
  const result = locales.map((locale) => {
    const fileReports = referenceLocaleNamespaces.map((referenceNamespace) => {
      const referenceFilePath = path.join(
        ...
        `${referenceNamespace}.json`,
      );
      const referenceNamespaceContent = safeParse(
        getTranslationFileContent({
      ...
      let localeNamespace = {};
      try {
        localeNamespace = safeParse(
          getTranslationFileContent({ namespace: referenceNamespace, locale }),
          localeNamespacePath,
        );
      } catch (e) {
        //Ignore error
  ...
  switch (config.mode) {
    ...
    case "write": {
      for (const localeResult of result) {
        const files = localeResult.files.filter(
          (file) => file.missingKeys.length > 0,
        );
...
export function getMissingNamespaces() {
  ...
  return locales.flatMap((locale) => {
    ...
    const missing = namespaces.filter(
      (namespace) => !referenceLocaleNamespaces.includes(namespace),
    );
